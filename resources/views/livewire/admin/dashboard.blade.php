...
<div class="max-w-5xl mx-auto px-4 sm:px-0 py-6 sm:py-8">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-5 mb-8">
    @foreach([
      ...
      ['label' => 'Pending', 'value' => $pending, 'href' => route('admin.referrals.pending'), 'accent' => true, 'delay' => '150'],
      ...
      <div class="bg-white rounded-2xl shadow-soft border border-white/80 overflow-hidden transition-all duration-300 hover:shadow-card flex flex-col w-full">
        <div class="p-5 sm:p-6 flex flex-col flex-1 min-h-0">
          ...
          <div class="h-64 sm:h-72 flex-1 min-h-[240px]" wire:ignore>
            <canvas id="chart-referrals-time" role="img" aria-label="Referrals over time chart"></canvas>
          </div>
      ...
      <div class="bg-white rounded-2xl shadow-soft border border-white/80 overflow-hidden transition-all duration-300 hover:shadow-card flex flex-col w-full h-full">
        <div class="p-5 sm:p-6 flex flex-col flex-1 min-h-0">
          ...
          <div class="h-64 sm:h-72 flex-1 min-h-[240px] flex items-center justify-center" wire:ignore>
            <canvas id="chart-status" role="img" aria-label="Referral status breakdown"></canvas>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="opacity-0 animate-fade-in-up delay-375">
    <div class="bg-white rounded-2xl shadow-soft border border-white/80 overflow-hidden transition-all duration-300 hover:shadow-card">
      <div class="p-5 sm:p-6">
        <h2 class="text-title font-semibold text-primaryText mb-4">Top referrers</h2>
        @if($topReferrers->isNotEmpty())
          <div class="h-48 sm:h-56 mb-4" wire:ignore>
            <canvas id="chart-top-referrers" role="img" aria-label="Top referrers bar chart"></canvas>
          </div>
          <ul class="space-y-2 border-t border-gray-100 pt-4">
            @foreach($topReferrers as $index => $r)
              <li class="flex items-center gap-3 py-2 text-body text-primaryText">
                <span class="w-6 h-6 rounded bg-brandBlue/10 text-subtitle font-semibold text-brandBlue flex items-center justify-center shrink-0">{{ $index + 1 }}</span>
                <span class="truncate flex-1">{{ $r->referrer->name ?? 'Unknown' }}</span>
                <span class="font-medium text-brandBlue shrink-0">{{ $r->cnt }} {{ $r->cnt === 1 ? 'referral' : 'referrals' }}</span>
              </li>
            @endforeach
          </ul>
        @else
          <p class="text-body text-secondaryText py-4">No referrals yet.</p>
        @endif
      </div>
    </div>
  </div>
...
<script>
  ...
  function initCharts() {
    ...
    const referrerLabels = @js($chartReferrerLabels);
    const referrerValues = @js($chartReferrerValues);

    // Referrals over time (line)
    const timeCtx = document.getElementById('chart-referrals-time');
    if (timeCtx && typeof Chart !== 'undefined') {
      if (window.adminTimeChart) window.adminTimeChart.destroy();
      window.adminTimeChart = new Chart(timeCtx, {
        type: 'line',
        data: {
          labels: timeLabels,
          datasets: [{
    ...
    if (statusCtx && typeof Chart !== 'undefined') {
      if (window.adminStatusChart) window.adminStatusChart.destroy();
      window.adminStatusChart = new Chart(statusCtx, {
        type: 'doughnut',
        data: {
          labels: statusLabels,
          datasets: [{
    ...
    const referrersCtx = document.getElementById('chart-top-referrers');
    if (referrersCtx && referrerLabels.length > 0 && typeof Chart !== 'undefined') {
      if (window.adminReferrersChart) window.adminReferrersChart.destroy();
      window.adminReferrersChart = new Chart(referrersCtx, {
        type: 'bar',
        data: {
  ...
  $wire.$watch('period', () => {
    $wire.getChartTimeData().then((data) => {
      if (window.adminTimeChart && data && data.labels) {
        window.adminTimeChart.data.labels = data.labels;
        window.adminTimeChart.data.datasets[0].data = data.values;
        window.adminTimeChart.update();
      }
    });
  });
</script>
